...
class AutoSubv2AV(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "autosubv2av"
    # 加载顺序
    plugin_order = 15
    # 可使用的用户级别
    auth_level = 2
    ...
    _DEFAULT_BOARD_LANG = {
        '霓虹': 'ja', '动漫': 'ja', '欧美': 'en',
        '韩国': 'ko', '探花': 'zh', '传媒': 'zh', '三级': 'zh',
    }
    # 默认扫描目录（7 个正式库）
    _DEFAULT_PATH_LIST = [
        '/vol2/1000/1024/霓虹', '/vol2/1000/1024/欧美', '/vol2/1000/1024/动漫',
        '/vol2/1000/1024/传媒', '/vol2/1000/1024/三级', '/vol2/1000/1024/韩国',
        '/vol2/1000/1024/探花',
    ]
    # 中文语言代码
    ...
    _LANG_MAP = {
        'lang_en': (['en', 'eng'], True),
        'lang_ja': (['ja', 'jpn'], True),
        'lang_zh': (['zh', 'chi', 'zho'], True),
        'lang_ko': (['ko', 'kor'], True),
        'lang_es': (['es', 'spa'], True),
        'lang_fr': (['fr', 'fre', 'fra'], True),
        'lang_de': (['de', 'ger', 'deu'], True),
    }
    ...
    def init_plugin(self, config=None):
        ...
        self._custom_language = config.get('custom_language', '')
        ...
        # ===== 成人特调配置 =====
        self._board_filter = [x.strip() for x in (config.get('board_filter') or '').replace('，', ',').split(',') if x.strip()]
        self._lang_source = config.get('lang_source', 'nfo_first')
        # 板块语言映射：从配置读（JSON 字符串或 dict），否则用默认
        _blm = config.get('board_lang_map')
        if isinstance(_blm, dict) and _blm:
            self._board_lang_map = _blm
        elif isinstance(_blm, str) and _blm.strip():
            try:
                import json as _json
                self._board_lang_map = _json.loads(_blm)
            except Exception:
                self._board_lang_map = dict(self._DEFAULT_BOARD_LANG)
        else:
            self._board_lang_map = dict(self._DEFAULT_BOARD_LANG)
        try:
            self._ad_skip_seconds = int(config.get('ad_skip_seconds', 300))
        except Exception:
    ...
    # ==================== 成人特调新增方法 ====================
    def __board_of(self, video_file: str) -> str:
        """从视频路径推断所属板块（/vol2/1000/1024/<板块>/...）"""
        try:
            parts = Path(video_file).parts
            # 找 '1024' 后一层
            for i, p in enumerate(parts):
                if p == '1024' and i + 1 < len(parts):
                    return parts[i + 1]
        except Exception:
            pass
        return ''

    def __pass_board_filter(self, video_file: str) -> bool:
        """板块过滤：配置为空则全部通过"""
        if not self._board_filter:
            return True
        return self.__board_of(video_file) in self._board_filter

    def __nfo_path_of(self, video_file: str) -> str:
        """同目录下的 .nfo 文件路径（优先与视频同名的）"""
        d = os.path.dirname(video_file)
        base = os.path.splitext(os.path.basename(video_file))[0]
        # 优先同名 nfo
        cand = os.path.join(d, base + '.nfo')
    ...
    def __read_nfo(self, video_file: str) -> dict:
        """读取同目录 nfo 的关键字段：countrycode / studio / originaltitle / tags"""
        res = {'countrycode': '', 'studio': '', 'originaltitle': '', 'tags': [], 'exists': False}
        nfo = self.__nfo_path_of(video_file)
        if not nfo:
            return res
        try:
            txt = open(nfo, encoding='utf-8', errors='replace').read()
            res['exists'] = True
            m = re.search(r'<countrycode>(.*?)</countrycode>', txt)
            if m:
                res['countrycode'] = m.group(1).strip().upper()
            m = re.search(r'<studio>(.*?)</studio>', txt)
            if m:
                res['studio'] = m.group(1).strip()
            m = re.search(r'<originaltitle>(.*?)</originaltitle>', txt)
            if m:
                res['originaltitle'] = m.group(1).strip()
            res['tags'] = re.findall(r'<tag>(.*?)</tag>', txt)
        except Exception:
            pass
        return res

    def __nfo_has_chinese_sub(self, video_file: str) -> bool:
        """NFO 是否标注了中文字幕（有则跳过，不用生成）"""
        nfo = self.__read_nfo(video_file)
        for t in nfo.get('tags', []):
            if '中文字幕' in t or '中字' in t or '字幕' in t and '中文' in t:
                return True
        return False

    @staticmethod
    def __has_cjk(s: str) -> bool:
        """是否含中日韩文字"""
        return bool(re.search(r'[\u3040-\u30ff\u4e00-\u9fff\uac00-\ud7af]', s or ''))

    @staticmethod
    def __has_kana(s: str) -> bool:
        """是否含日文假名（判定日语最可靠）"""
    ...
    def __board_lang(self, video_file: str) -> str:
        """按板块取期望语言"""
        b = self.__board_of(video_file)
        return (self._board_lang_map or {}).get(b, '')

    def __resolve_lang(self, video_file: str) -> str:
        """综合判断语言（返回 iso639-1 代码，'' 表示让 whisper auto）"""
        mode = self._lang_source or 'nfo_first'
        if mode == 'board':
            return self.__board_lang(video_file)
        if mode == 'nfo_first':
            lang = self.__detect_lang_from_nfo(video_file)
            if lang:
                return lang
            return self.__board_lang(video_file)
        # auto
    ...
    def __is_chinese_lang(self, lang: str) -> bool:
        return (lang or '').lower() in self._ZH_LANGS
    ...
    def __process_autosub(self, video_file) -> TaskStatus:
        ...
        # ===== 成人特调：板块过滤 =====
        if not self.__pass_board_filter(video_file):
            logger.info(f"[板块过滤] 不在处理范围，跳过：{video_file}")
            return TaskStatus.IGNORED

        # ===== 成人特调：NFO 已标注中文字幕 → 跳过 =====
        if self._skip_nfo_chinese and self.__nfo_has_chinese_sub(video_file):
            logger.info(f"[NFO中文字幕] 已标注中文字幕，跳过：{video_file}")
            return TaskStatus.IGNORED
    ...
    def __do_speech_recognition(self, audio_lang, audio_file, clip_start=0):
        """
        语音识别, 生成字幕
        :param audio_lang:
        :param audio_file:
        :param clip_start: 起始秒数（>0 时用 clip_timestamps 跳过开头广告）
        :return:
        """
        lang = audio_lang
        try:
    ...
    def __generate_subtitle(self, video_file, subtitle_file, enable_asr=True):
        ...
        # 获取字幕语言偏好
        lang_entry = self._LANG_MAP.get(self._translate_preference)
        if lang_entry:
            prefer_subtitle_langs, strict = lang_entry
        elif self._translate_preference == "origin_first":
            prefer_subtitle_langs = None
            strict = False
        elif self._translate_preference == "custom":
            if self._custom_language:
                try:
                    lang_code = iso639.to_iso639_1(self._custom_language)
                    prefer_subtitle_langs = [lang_code, self._custom_language]
                except iso639.NonExistentLanguageError:
                    prefer_subtitle_langs = [self._custom_language]
            else:
        ...
        # 仅当 NFO/板块都判不出时，才用 whisper 自动检测
            ...
            # 如果偏好为特定语言，强制使用该语言进行ASR
            if self._translate_preference in self._LANG_MAP:
                prefer_langs, _ = self._LANG_MAP[self._translate_preference]
                audio_lang = prefer_langs[0]  # 使用偏好语言
                logger.info(f"强制使用偏好语言：{audio_lang} 进行ASR识别")
            else:
                audio_lang = 'auto'
        ...
        is_strict_lang = self._translate_preference in self._LANG_MAP or self._translate_preference == "custom"
        if is_strict_lang:
            # 严格语言模式：只接受完全匹配的语言，不降级
    ...
    def __target_subtitle_exists(self, video_file):
        ...
        if getattr(self, '_check_zh_subtitle', True) or self._translate_zh:
            ...
            lang_entry = self._LANG_MAP.get(self._translate_preference)
            if lang_entry:
                prefer_langs, strict = lang_entry
            elif self._translate_preference == "custom":
                if self._custom_language:
                    try:
                        lang_code = iso639.to_iso639_1(self._custom_language)
                        prefer_langs = [lang_code, self._custom_language]
                    except iso639.NonExistentLanguageError:
                        prefer_langs = [self._custom_language]
                else:
                    prefer_langs = ['en', 'eng']
                strict = True
            else:
                ...
                'content': [
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'model': 'board_filter',
                                            'label': '板块过滤',
                                            'placeholder': '留空=全部；多个用逗号分隔，如：霓虹,欧美'
                                        }
                                    }
                                ]
                            },
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'label': '语言判定来源',
                                            'items': [
                                                {'title': 'NFO优先（推荐）', 'value': 'nfo_first'},
                    ...
                    },
                    {
                        ...
                        'content': [
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'items': [
                                                ...
                                                {'title': '仅英语', 'value': 'lang_en'},
                                                {'title': '仅日语', 'value': 'lang_ja'},
                                                {'title': '仅中文', 'value': 'lang_zh'},
                                                {'title': '仅韩语', 'value': 'lang_ko'},
                                                {'title': '仅西班牙语', 'value': 'lang_es'},
                                                {'title': '仅法语', 'value': 'lang_fr'},
                                                {'title': '仅德语', 'value': 'lang_de'},
                            ...
                            {
                                'component': 'VCol',
                                'props': {'cols': 12, 'md': 4, 'v-show': "translate_preference === 'custom'"},
                                'content': [
                                    {
                                        'component': 'VTextField',
                                        'props': {
                                            'model': 'custom_language',
                                            'label': '自定义语言代码',
                                            'placeholder': '例如: en, ja, ko, fr, de, es',
                                            'hint': '输入ISO 639-1语言代码，如en(英语)、ja(日语)'
                                        }
                                    }
                                ]
                            },
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'label': '计算类型',
                                            'items': [
                                                {'title': 'int8（快速，推荐）', 'value': 'int8'},
                                                {'title': 'float32（高精度）', 'value': 'float32'}
                                            ],
                                            'hint': 'Whisper推理计算精度，int8速度更快，float32精度更高'
                                        }
                                    }
                                ]
                            }
