...
class AutoSignIn(_PluginBase):
    ...
    plugin_name = "站点自动签到"
    # 插件描述
    plugin_desc = "自动模拟登录、签到站点。（本地改造：仅失败时推送摘要，签到与登录合并为一条通知）"
    # 插件图标
    ...
    def sign_in(self, event: Event = None):
        ...
        # 本地改造（9.9.10）：签到与登录的异常摘要先收集，最后合并成一条推送，避免收到过多通知
        notify_holder: List[dict] = []
        if self._sign_sites:
        ...
        if self._login_sites:
            self.__do(today=today, type_str="登录", do_sites=self._login_sites, event=event,
                      notify_holder=notify_holder)
        self.__send_merged_notify(notify_holder)

    def __do(self, today: datetime, type_str: str, do_sites: list, event: Event = None,
             notify_holder: Optional[list] = None):
        """
        签到逻辑

        :param notify_holder: 本地改造（9.9.10）通知收集容器；不为 None 时把异常摘要收集进去，
                              由 sign_in 统一合并推送，避免签到/登录各推一条通知
        """
        ...
        # 本地改造：单站点硬超时，避免个别站点卡死导致整个任务永不结束
        _site_func = self.signin_site if type_str == "签到" else self.login_site
        _site_timeout = 90
        _executor = ThreadPoolExecutor(max_workers=min(len(do_sites), int(self._queue_cnt)))
        try:
            _futures = {_executor.submit(_site_func, s): s for s in do_sites}
            status = []
            for _fut, _site in _futures.items():
                _name = _site.get("name") if isinstance(_site, dict) else str(_site)
                try:
                    status.append(_fut.result(timeout=_site_timeout))
                except FuturesTimeout:
                    logger.warn(f"{_name} {type_str}超时（>{_site_timeout}秒），跳过等待其余响应")
                    status.append((_name, f"{type_str}失败：超时（>{_site_timeout}秒）"))
                except Exception as _e:
                    status.append((_name, f"{type_str}失败：{str(_e)[:80]}"))
        finally:
        ...
        if status:
            ...
            # 收集通知（本地改造：仅推送失败摘要，全部成功则不推送；
            # 9.9.10 起统一交给 sign_in 把签到/登录合并成一条推送）
            if self._notify:
                total = len(self._sign_sites if type_str == '签到' else self._login_sites)
                # 失败/命中重试的站点（"已签到/无需再签"属于正常状态，不算失败）
                error_msg = [s for s in (failed_msg + retry_msg)
                             if s and "已签到" not in str(s[1]) and "无需再签" not in str(s[1])]
                if error_msg:
                    error_sites = {str(s[0]) for s in error_msg}
                    signin_message = "\n".join([f'【{s[0]}】{s[1]}' for s in error_msg])
                    if notify_holder is not None:
                        # 交给 sign_in 汇总后统一推送
                        notify_holder.append({
                            "type": type_str,
                            "count": len(do_sites),
                        ...
                        # 兜底：未经 sign_in 汇总时，保持原有单独推送行为
                        self.post_message(
                            title=f"【站点自动{type_str}】{len(error_sites)}个异常",
                            mtype=NotificationType.SiteMessage,
                            text=f"全部{total}个，本次{type_str}{len(do_sites)}个，异常{len(error_sites)}个：\n"
                                 f"{signin_message}"
                        )
                else:
                    logger.info(f"站点自动{type_str}全部正常，按降噪设置本次不推送")
            if event:
                self.post_message(channel=event.event_data.get("channel"),
    ...
    def __send_merged_notify(self, notify_holder: list):
        """
        本地改造（9.9.10）：把签到/登录的异常摘要合并成一条通知推送

        :param notify_holder: sign_in 收集的通知条目列表，每项形如
                              {"type": 类型, "count": 本次执行站点数, "error_sites": {站点名}, "text": 摘要}
        """
        if not notify_holder:
            # 签到与登录都没有异常（或未开启通知）→ 不推送，保持降噪
            return
        # 汇总所有异常站点（跨签到/登录去重计数）
        all_error_sites = set()
        for item in notify_holder:
            all_error_sites |= set(item.get("error_sites") or [])
        # 按类型分节拼接正文
        sections = []
        for item in notify_holder:
            sections.append(
                f"【{item.get('type')}】本次{item.get('count')}个，"
                f"异常{len(item.get('error_sites') or [])}个：\n{item.get('text')}"
            )
        types = "/".join(str(item.get("type")) for item in notify_holder)
        self.post_message(
            title=f"【站点自动{types}】{len(all_error_sites)}个站点异常",
            mtype=NotificationType.SiteMessage,
            text="\n\n".join(sections)
        )
