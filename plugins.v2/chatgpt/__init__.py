...
class ChatGPT(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if model_source not in {self.MODEL_SOURCE_SYSTEM, self.MODEL_SOURCE_AGENT_TOKENS, self.MODEL_SOURCE_CUSTOM}:
        ...
        self._custom_api_key = self._clean_text(config.get("custom_api_key"))
        self._custom_base_url = self._clean_text(config.get("custom_base_url"))
        self._custom_model = self._clean_text(config.get("custom_model"))
        self._custom_provider = self._clean_text(config.get("custom_provider")) or "openai"
        self.openai = None
        ...
        return [
            {
                ...
                "content": [
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "items": [
                                                ...
                                                {"title": "自定义 LLM 配置", "value": self.MODEL_SOURCE_CUSTOM},
                                            ],
                                            "hint": "选择 Agent Tokens 时，插件会发出 Agent LLM 供应商链式事件，并读取插件返回的 API Base URL、API Key 与模型 ID。选择自定义时，使用下方独立填写的 LLM 配置。",
                                            "persistent-hint": True,
                                        },
                                    }
                                ],
                    ...
                    {
                        "component": "VRow",
                        "props": {"v-if": f"model_source === '{self.MODEL_SOURCE_CUSTOM}'"},
                        "content": [
                            {
                                "component": "VCol",
                                "props": {"cols": 12, "md": 6},
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "model": "custom_base_url",
                                            "label": "自定义 API Base URL",
                                            "placeholder": "https://api.siliconflow.cn/v1",
                                            "hint": "兼容 OpenAI 的 API 地址",
                                            "clearable": True,
                                            "persistent-hint": True,
                                        },
                                    }
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "model": "custom_model",
                                            "label": "自定义模型 ID",
                                            "placeholder": "Pro/Qwen/Qwen2-72B-Instruct",
                                            "hint": "使用的模型名称或路径",
                                            "clearable": True,
                                            "persistent-hint": True,
                                        },
    ...
    def _resolve_custom_model_config(self) -> Tuple[Optional[Dict[str, Any]], str]:
        """
        使用插件配置中独立填写的 LLM 运行参数，不依赖系统设置或 Agent Tokens。
        """
        config = {
            "provider": self._custom_provider or "openai",
            "model": self._custom_model,
            "api_key": self._custom_api_key,
            "base_url": self._custom_base_url or None,
    ...
    def _resolve_model_config(self) -> Tuple[Optional[Dict[str, Any]], str]:
        ...
        elif self._model_source == self.MODEL_SOURCE_CUSTOM:
            return self._resolve_custom_model_config()
